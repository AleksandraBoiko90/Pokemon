...
function savePokemon(pokemon) {
  ...
  const isPokemonExist = pokemons.some(existingPokemon => 
      existingPokemon.name === pokemon.name && existingPokemon.type === pokemon.type
  );
...
function fetchPokemons(type = '') {
  ...
  let pokemonsToShow = [...customPokemons, ...allPokemons].filter(pokemon => !type || pokemon.type === type);
  ...
  .then(data => {
      ...
      Promise.all(promises).then(results => {
          ...
          const allPokemons = [...pokemonsToShow, ...filteredResults];
